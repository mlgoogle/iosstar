...
//        var addresses = [String]()
    ...
    static func bankCardName(bankCardNum: String) -> String {
        ...
        var bankIndex = -1
        let bankCard: NSString = bankCardNum as NSString
        let bankCardBin = bankCard.substring(to: 6)
        for (index, bankCode) in bankBin.enumerated(){
            if bankCardBin == bankCode{
                bankIndex = index
                break
            }
        }
        if bankIndex != -1 {
            return bankName[bankIndex]
        }
        
        let bankCardBin_8 = bankCard.substring(to: 8)
        for (index, bankCode) in bankBin.enumerated(){
            if bankCardBin_8 == bankCode{
                bankIndex = index
                break
            }
        }
