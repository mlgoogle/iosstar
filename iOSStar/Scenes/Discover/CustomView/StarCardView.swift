...
//        
    ...
    func setStarModel(starModel:StarSortListModel) {
        guard URL(string:qiniuHelper.shared().qiniuHeader +  starModel.home_pic) != nil else {
            return
        }
        
        showImageView.kf.setImage(with: URL(string:qiniuHelper.shared().qiniuHeader +  starModel.home_pic)!, placeholder: UIImage(named: "blank"), options: nil, progressBlock: nil) { (image, error, cacheType, url) in
            if image != nil {
