...
class HeatListCell: OEZTableViewCell {
    ...
    override func update(_ data: Any!) {
        if  let model = data as? StarSortListModel {
            iconImageView.kf.setImage(with: URL(string:ShareDataModel.share().qiniuHeader + model.pic), placeholder: nil, options: nil, progressBlock: nil, completionHandler: nil)
